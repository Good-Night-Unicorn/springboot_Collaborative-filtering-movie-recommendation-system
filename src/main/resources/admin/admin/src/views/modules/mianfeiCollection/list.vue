        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '免费电影名称' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.mianfeiName" placeholder="免费电影名称" clearable></el-input>
                    </el-form-item>
                                                                                                                         
                    <el-form-item :label="contents.inputTitle == 1 ? '免费电影类型' : ''">
                        <el-select v-model="searchForm.mianfeiTypes" placeholder="请选择免费电影类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                                    v-for="(item,index) in mianfeiTypesSelectSearch"
                                    v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('mianfeiCollection','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('mianfeiCollection','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('mianfeiCollection','报表')"
                                type="success"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('mianfeiCollection','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/dianyingtuijianxitong/upload/mianfeiCollectionMuBan.xls"
                        >批量导入免费电影收藏数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('mianfeiCollection','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="mianfeiCollectionUploadSuccess"
                                :on-error="mianfeiCollectionUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('mianfeiCollection','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入免费电影收藏数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('mianfeiCollection','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "mianfeiCollection.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('mianfeiCollection','查看')"
                          :data="dataList"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            {{scope.row.mianfeiName}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="mianfeiPhoto"
                               header-align="center"
                               width="200"
                               label="免费电影照片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.mianfeiPhoto">
                                <img :src="$base.url+scope.row.mianfeiPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="mianfeiCollectionTypes"
                                      header-align="center"
                                      label="类型">
                        <template slot-scope="scope">
                            {{scope.row.mianfeiCollectionValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('mianfeiCollection','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('mianfeiCollection','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('mianfeiCollection','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    export default {
        ...
        return {
            ...
            userId:"",//当前登录人的id
    //级联表下拉框搜索条件
              mianfeiTypesSelectSearch : [],
    //当前表下拉框搜索条件
            form:{
                id : null,
                mianfeiId : null,
                yonghuId : null,
                mianfeiCollectionTypes : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     '免费电影名称': 'mianfeiName',
                     '免费电影编号': 'mianfeiUuidNumber',
                     '免费电影照片': 'mianfeiPhoto',
                     '赞': 'zanNumber',
                     '踩': 'caiNumber',
                     '电影': 'mianfeiVideo',
                     '免费电影类型': 'mianfeiTypes',
                     '免费电影热度': 'mianfeiClicknum',
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'mianfei_collection',//当前表表名,
                        sumColum : 'mianfei_collection_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'mianfei_collection_name',//分组字符串字段
                        // types : 'mianfei_collection_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "mianfei_collection_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                let params = {
                    ...
                    limit: this.pageSize,
                    sort: 'id',
                    mianfeiCollectionTypes: 1,
                }

                                         
                if (this.searchForm.mianfeiName!= '' && this.searchForm.mianfeiName!= undefined) {
                    params['mianfeiName'] = '%' + this.searchForm.mianfeiName + '%'
                }
                                                                                                                         
                if (this.searchForm.mianfeiTypes!= '' && this.searchForm.mianfeiTypes!= undefined) {
                    params['mianfeiTypes'] = this.searchForm.mianfeiTypes
                }
                ...
                params['mianfeiCollectionDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "mianfeiCollection/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                //查询级联表搜索条件所有列表
                this.$http({
                    url: "dictionary/page?dicCode=mianfei_types&page=1&limit=100",
                    method: "get",
                    page: 1,
                    limit: 100,
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.mianfeiTypesSelectSearch = data.data.list;
                    }
                });
                //查询当前表搜索条件所有列表
            },
            ...
            // 导入功能上传文件成功后调用导入方法
            mianfeiCollectionUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "mianfeiCollection/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
                            message: "导入免费电影收藏数据成功",
                            type: "success",
                            duration: 1500,
                            onClose: () => {
                                _this.search();
            ...
            // 导入功能上传文件失败后调用导入方法
            mianfeiCollectionUploadError(data){
